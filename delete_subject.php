...
if(isset($_POST['delete'])) {
  ...
  for ($i = 0; $i < count($_POST["riyu"]); $i++){
    ...
    $delete_subject_name = $_POST["riyu"][$i];

    // DELETEのSQL作成
    $sql = "DELETE FROM $username WHERE subject_name = '$delete_subject_name'";
...
<html>
<head>
<title>履修済科目削除｜広島市立大学 履修確認一覧サービス</title>
...
<tr>
  ...
  <td class="subject"><input type="checkbox" name="riyu[]" value="<?php echo $subject_name; ?>" <?php echo "$checked" ?>/></td>
</tr>
</table>
  <?php
}
...
<button type="submit" class="btn btn-default" name="delete" value="1">削除する</button></br>
</form>
</div>
